...
import cats.{Applicative, ApplicativeError, FlatMap, MonadError}
import goldrush.Constants.{ThrowableApplicativeError, ThrowableMonadError}
import goldrush._
...
class FakeClient[F[_]: ThrowableMonadError: Timer] extends Client[F] {
  ...
  def explore2(area: Area): F[ExploreResponse] = {
  ...
  override def explore(area: Area): F[ExploreResponse] = {
    fakeWaitAndResponse(
      Right(ExploreResponse(area, Random.nextInt(5) + 1)),
      30,
      5,
      methodName = "explore"
    )
...
object FakeClient {
  ...
  private def fakeWaitAndResponse[T, F[
    ...
    ApplicativeError[F, Throwable]
      .handleErrorWith(response)(th => ApplicativeError[F, Throwable].raiseError(printVal(th)))
  }
