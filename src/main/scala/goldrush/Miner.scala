...
case class Miner[F[_] : Sync : Parallel : Applicative](client: Client[F]) {
  def mine: F[Int] = {
    val licenses: Iterant[F, String] = Iterant[F].repeatEvalF(client.issueLicense())
      .mapBatch { license =>
    ...
    val explorator = {
      ...
      coords.mapEval {
        case (x, y) => client.explore(Area(x, y, 1, 1))
      }.filter(_.amount > 0).map { result =>
    ...
    val digger = {
      ...
      explorator.mapEval {
          ...
          licenses.foldWhileLeftEvalL(seed) {
            case ((level, foundTreasures), license) =>
              client.dig(license, x, y, level).map { newTreasures =>
                ...
                Either.cond(level >= 9 || nextTreasures.size >= amount,
                  result,
                  result
                )
              }
        ...
        case (_, treasures) => Batch.fromSeq(treasures)
      }
    }

    val coins = digger.mapEval { treasure =>
