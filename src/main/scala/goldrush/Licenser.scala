...
import goldrush.Counters.{freeLicenceCount, freeLicenceDigsCount, paidLicenceDigsCount, paidLicenceTriesCount}
...
object Licenser {
  ...
  object Issuer {
    def free[F[_]: Monad](client: Client[F]): Issuer[F] = {
      for {
        _ <- Applicative[F].pure(freeLicenceCount.incrementAndGet())
